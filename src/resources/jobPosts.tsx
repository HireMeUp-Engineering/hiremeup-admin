...
const jobPostExporter = (records: any[]) => {
  ...
  link.setAttribute('download', `job-posts-${new Date().toISOString().split('T')[0]}.csv`);
      ...
      <FunctionField
        ...
        render={(record: any) => (
          <Box display="flex" alignItems="center" gap={0.5}>
            ...
            <Typography variant="body2">{formatEmploymentType(record.employmentType)}</Typography>
          </Box>
        )}
      />
      ...
      <FunctionField
        ...
        render={(record: any) => (
          <Box display="flex" alignItems="center" gap={0.5}>
            ...
            <Typography variant="body2" title={new Date(record.createdAt).toLocaleString()}>
          ...
          <Box sx={{ mb: 2 }}>
            <Card sx={{ mb: 2 }}>
              ...
              <Box sx={{ p: 2 }}>
                <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
                  ...
                  <Box>
                    <strong>Location:</strong> {record.city ? `${record.address || ''} ${record.city}, ${record.zip || ''}`.trim() : 'N/A'}
                  </Box>
                  <Box>
                    <strong>Employment Type:</strong> {record.employmentType || 'N/A'}
                  </Box>
                  <Box>
                    <strong>Views:</strong> {record.viewCount || 0}
                  </Box>
                  <Box>
                    <strong>Applications:</strong> {record.applicationCount || 0}
                  </Box>
                  <Box>
            ...
            <Card sx={{ mb: 2 }}>
              ...
              <Box sx={{ p: 2 }}>
                <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
                  <Box>
                    <strong>Name:</strong> {record.poster?.firstName} {record.poster?.lastName}
                  </Box>
                  <Box>
                  ...
                  <Box>
                    <strong>Active:</strong> {record.poster?.isActive ? 'Yes' : 'No'}
                  </Box>
                </Box>
              </Box>
            </Card>
            ...
            <Card sx={{ mb: 2 }}>
              ...
              <Box sx={{ p: 2 }}>
                <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
                  <Box>
                    <strong>Pay Type:</strong> {record.isSalary ? 'Salary' : record.isHourly ? 'Hourly' : 'N/A'}
                  </Box>
                  <Box>
                    <strong>Pay Range:</strong> {
                      record.payRangeMin && record.payRangeMax
                        ? `${record.payCurrency || '$'}${record.payRangeMin} - ${record.payCurrency || '$'}${record.payRangeMax}`
                        : 'Not specified'
                    }
                  </Box>
                  <Box>
            ...
            <Card sx={{ mb: 2 }}>
              ...
              <Box sx={{ p: 2 }}>
                {record.jobDescription ? (
                  <div dangerouslySetInnerHTML={{ __html: record.jobDescription }} />
                ) : (
            ...
            {record.projectDescription && (
              <Card sx={{ mb: 2 }}>
                ...
                <Box sx={{ p: 2 }}>
                  <div dangerouslySetInnerHTML={{ __html: record.projectDescription }} />
                </Box>
              </Card>
            )}
              ...
              <Card sx={{ mb: 2 }}>
                ...
                <Box sx={{ p: 2 }}>
                  ...
                  {record.mainAudioUrl && !record.mainVideoUrl && (
                    <Box sx={{ mb: 2 }}>
                      <audio src={record.mainAudioUrl} controls style={{ width: '100%' }} />
                    </Box>
                  )}
            ...
            {record.applications && record.applications.length > 0 && (
              <Card>
                <Box sx={{ p: 2, bgcolor: '#f5f5f5', borderBottom: '1px solid #e0e0e0' }}>
                  <h3 style={{ margin: 0 }}>Applications ({record.applications.length})</h3>
                </Box>
                <Box sx={{ p: 2 }}>
                    ...
                    >
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Box>
                          ...
                          <br />
                          <span style={{ fontSize: '0.875rem', color: '#666' }}>{app.applicantEmail}</span>
                        </Box>
                        <Box sx={{ textAlign: 'right' }}>
                          <Chip label={app.status} size="small" sx={{ mb: 0.5 }} />
                          <br />
